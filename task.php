...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    } elseif (count($dataMatch) != 3 ||
        strlen($dataMatch[0]) != 2 ||
        strlen($dataMatch[1]) != 2 ||
        strlen($dataMatch[2]) != 4 ||
        !checkdate($dataMatch[0] , $dataMatch[1] , $dataMatch[2])){
        $error_message = 'Invalid Date';
    } else {
        if (add_tasks($project_id, $title, $date, $time, $task_id)) {
            ...
            <form class="form-container form-add" method="post" action="task.php">
                <table>
                    ...
                    <tr>
                        ...
                        <td><input type="text" id="date" name="date" value="<?php echo htmlspecialchars($date); ?>"
                                   placeholder="mm/dd/yyyy"/></td>
                    </tr>
                    <tr>
                        <th><label for="time">Time<span class="required">*</span></label></th>
                        <td><input type="text" id="time" name="time" value="<?php echo htmlspecialchars($time); ?>"/>
                            minutes
                        </td>
                    </tr>
                </table>
